...
describe('Author() Author method', () => {
  ...
  describe("Security tests", () => {
    test("does not render link if github URL is not a valid URL", () => {
      render(
        <Author
          name="Invalid URL"
      ...
      const name = screen.getByText("Invalid URL");
      expect(name).toBeInTheDocument();
      expect(screen.queryByRole("link", { name: "Invalid URL" })).not.toBeInTheDocument();
    });

    test("does not render link if github URL is a data URL", () => {
      render(
        <Author
          name="Data URL"
          github="data:text/html;base64,PHNjcmlwdD5hbGVydCgnWFNTJyk8L3NjcmlwdD4="
        />
      );
      const name = screen.getByText("Data URL");
      expect(name).toBeInTheDocument();
      expect(screen.queryByRole("link", { name: "Data URL" })).not.toBeInTheDocument();
    });

    test("does not render link if github URL is a file URL", () => {
      render(
        <Author
          name="File URL"
      ...
      const name = screen.getByText("File URL");
      expect(name).toBeInTheDocument();
